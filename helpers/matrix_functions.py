...
@torch.no_grad()
def power_iteration(mat_g, error_tolerance=1e-6, num_iters=100):
    """Power iteration.
...
@torch.no_grad()
def compute_inv_p_power_coupled_newton(mat_g, p, iter_count=100, error_tolerance=1e-8, ridge_epsilon=1e-8):
    ...
    if shape[0] == 1:
        return identity
    alpha = -1.0 / p
    max_ev, _, _ = power_iteration(mat_g)
...
@torch.no_grad()
def compute_matrix_power_evd(A, p, t):
    if t == 'evd':
        L, V = torch.linalg.eig(A)
        L, V = L.type(torch.float), V.type(torch.float)
    elif t == 'evdh':
        L, V = torch.linalg.eigh(A)
    inv_sqrt_L = L.unsqueeze(1) ** p
    inv_sqrt_A = mm(V, V.T * inv_sqrt_L)
    return inv_sqrt_A
    """
        Old code from precondition:
        
        def _precondition(self, x):
            const = self.eps * self._unbiasing_term()
            L, V = EVD(self.buffer + const * self.eye)
            L, V = L.type(torch.float), V.type(torch.float)
            invSqrtL = L.unsqueeze(1) ** -0.5
            inv_sqrt_mul_g = mm(V, mm(V.T * invSqrtL, x))
            return inv_sqrt_mul_g
    """


@torch.no_grad()
def newton_iteration_naive(A, max_iters=100):
    n = A.size()[0]

...
class Trainer:
    ...
    def create_optimizer_and_scheduler(self, steps_per_epoch: int, num_train_epochs: int, num_training_steps: int):
        """
        ...
        self.create_scheduler(steps_per_epoch, num_train_epochs, num_training_steps)
    ...
    def create_scheduler(self, steps_per_epoch:int, num_train_epochs: int, num_training_steps: int):
        """
    ...
    def train(
        ...
        **kwargs,
        ...
        if args.deepspeed:
            ...
            self.create_optimizer_and_scheduler(steps_per_epoch=num_update_steps_per_epoch, num_train_epochs=num_train_epochs, num_training_steps=max_steps)
        ...
        if self.args.lr_scheduler_type == 'epoch-step':
            logger.info(f"  Decaying LR @ {list(range(num_update_steps_per_epoch, num_train_epochs*num_update_steps_per_epoch, num_update_steps_per_epoch))}")
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    if ionut_thresh is not None and tr_loss > ionut_thresh:
                        ionut_stop = True
            ...
            if ionut_stop:
                print(f'[IONUT] PROCESS STOPPED BECAUSE LOSS {tr_loss} > {ionut_thresh} @ STEP {self.state.global_step}')
                sys.exit(666)
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch, ignore_keys_for_eval):
        if self.control.should_log:
            ...
            if hasattr(self.optimizer, 'damp'):
                logs['damp'] = self.optimizer.hinv.damp
